...
class MiniSweAgent(IsolatedProviderAgent, HarborMiniSweAgent):
    ...
    async def _prepare_inference_env(
        ...
        if not bridged:
            raise RuntimeError("mini-swe-agent requires the Sandbox inference route")
        if "OPENAI_BASE_URL" not in env or "MSWEA_API_KEY" not in env:
            raise RuntimeError(
                "Sandbox inference route did not provide mini-swe-agent credentials"
            )
