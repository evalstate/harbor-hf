...
class KimiCodeAgent(IsolatedProviderAgent, KimiCode):
    ...
    async def _prepare_inference_env(
        ...
        if not bridged:
            raise RuntimeError("Kimi Code requires the Sandbox inference route")
        if "KIMI_MODEL_BASE_URL" not in env or "KIMI_MODEL_API_KEY" not in env:
            raise RuntimeError(
                "Sandbox inference route did not provide Kimi Code credentials"
            )
        env["KIMI_MODEL_NAME"] = self._allowed_model()
        return env
