...
from harbor_hf.planner import ExecutionCell, experiment_digest, resolved_cells
...
class ExecutionAdmission(FrozenModel):
    cell_digest: str
...
class PlannedExecution(ExecutionAdmission):
...
class RunRecoveryPolicy(FrozenModel):
    max_active_waves: int = Field(default=64, ge=1)
    max_physical_attempts_per_trial: int = Field(default=3, ge=1)
    ...
    def retry_backoff_is_bounded(self) -> RunRecoveryPolicy:
...
class RunPlan(FrozenModel):
    schema_version: Literal["harbor-hf/run-plan/v1alpha1"] = (
        "harbor-hf/run-plan/v1alpha1"
    )
    experiment: str
    ...
    controller_policy: RunControllerSpec | None = Field(
        default=None, exclude_if=lambda value: value is None
    )
    planned_run_duration_seconds: int | None = Field(
        default=None, ge=1, exclude_if=lambda value: value is None
    )
    ...
    @model_validator(mode="after")
    def counts_match_contents(self) -> RunPlan:
        ...
        if (self.controller_policy is None) != (
            self.planned_run_duration_seconds is None
        ) or (self.controller_policy is None) != (not self.initial_waves):
            raise ValueError("run controller planning fields are incomplete")
        ...
        shard_count = sum(len(execution.shards) for execution in self.executions)
        trial_count = sum(
            len(shard.trials)
            for execution in self.executions
            for shard in execution.shards
        )
        if (self.execution_count, self.shard_count, self.trial_count) != (
            len(self.executions),
            shard_count,
            trial_count,
        ):
            raise ValueError("run plan counts do not match its contents")
        return self


class RunTrialLock(FrozenModel):
    trial_id: str
    trial_digest: str
    task_name: str
    task_digest: str
...
class RunShardLock(FrozenModel):
    shard_id: str
    shard_digest: str
    trials: list[RunTrialLock]


class RunExecutionLock(ExecutionAdmission):
    execution_id: str
    shards: list[RunShardLock]


class RunLock(FrozenModel):
    schema_version: Literal["harbor-hf/run-lock/v1alpha1"] = (
    ...
    executions: list[RunExecutionLock]

    @model_validator(mode="after")
    def publication_role_is_consistent(self) -> RunLock:
        if (self.publication_role == "component") != (self.component_kind is not None):
            raise ValueError("component kind conflicts with publication role")
        if (self.controller_policy is None) != (
            self.planned_run_duration_seconds is None
        ) or (self.controller_policy is None) != (not self.initial_waves):
            raise ValueError("run controller lock fields are incomplete")
        if [wave.wave_index for wave in self.initial_waves] != list(
            range(1, len(self.initial_waves) + 1)
        ):
...
class WaveExecutionLock(FrozenModel):
    artifact_prefix: str
...
def estimated_partial_wave_cost(
    ...
    shard_sizes = sorted(
        (
            ...
            if execution.deployment_digest == deployment_digest
            for shard in execution.shards
        ),
...
class WaveLock(FrozenModel):
    ...
    executions: list[WaveExecutionLock]
...
def build_run_plan(
    ...
    *,
    ...
    selected_recovery_policy = recovery_policy or RunRecoveryPolicy()
    ...
    _validate_run_cells(cells, profiles)
    executions = [
        _plan_execution(resolved_spec, cell, trials, profiles, source_identity)
        for cell in cells
    ]
    controller_policy, initial_waves, planned_run_seconds = _controller_plan(
        resolved_spec, executions
    )
    plan_payload = {
        "schema_version": "harbor-hf/run-plan/v1alpha1",
        ...
        plan_payload.update(
            {
                ...
                "planned_run_duration_seconds": planned_run_seconds,
                "initial_waves": [
    ...
    return RunPlan(
        ...
        shard_count=sum(len(execution.shards) for execution in executions),
        trial_count=sum(
            len(shard.trials) for execution in executions for shard in execution.shards
        ),
...
def _resolved_tasks(spec: ExperimentSpec) -> list[tuple[str, str]]:
    ...
    if not digests:
        raise ValueError("run planning requires resolved task digests")
    if any(
        not any(
        ...
        raise ValueError("run task digests must exactly resolve task selections")
    return sorted(digests.items())


def _validate_run_cells(
    ...
    for cell in cells:
        ...
        if previous != pair:
            raise ValueError(
                "run deployment digest must resolve to one model and "
...
def _controller_plan(
    spec: ExperimentSpec, executions: list[PlannedExecution]
) -> tuple[
    RunControllerSpec | None,
    list[PlannedInitialWave],
    int | None,
]:
    policy = _provider_controller_policy(spec, executions)
    if policy is None:
        return None, [], None
    grouped_shards, concurrency = _provider_wave_groups(spec, executions)
    ...
    if remote is None or total > remote.job.timeout_seconds:
        raise ValueError(
            "planned provider run duration exceeds remote.job.timeout_seconds"
        )
...
def _provider_controller_policy(
    ...
    provider_count = sum(execution.provider is not None for execution in executions)
    if 0 < provider_count < len(executions):
        raise ValueError("runs cannot mix inference providers and inference endpoints")
    policy = spec.execution.controller
    if provider_count == 0:
        if policy is not None:
            raise ValueError(
                "execution.controller is only valid for inference-provider runs"
            )
        return None
    if policy is None:
        raise ValueError(
            "inference-provider runs require execution.controller settings"
        )
    if spec.remote is None:
        raise ValueError("provider run controllers require remote execution")
    return policy
...
def _provider_wave_groups(
    ...
    for execution in executions:
        provider_limit = execution.max_concurrent_requests
        if provider_limit is None:
            raise ValueError("provider execution has no request concurrency limit")
        effective = min(
            spec.execution.concurrent_trials,
            provider_limit,
            profile_limit,
        )
        previous = concurrency.setdefault(execution.deployment_digest, effective)
        if previous != effective:
            raise ValueError("provider deployment has inconsistent concurrency limits")
        grouped_shards.setdefault(execution.deployment_digest, []).extend(
            execution.shards
        )
...
def build_run_lock(
    ...
    *,
    ...
    for planned_execution in plan.executions:
        execution_id = _short_id(
            "execution",
            {"run_id": run_id, "cell_digest": planned_execution.cell_digest},
        )
        shards = []
        for planned_shard in planned_execution.shards:
            trials = [
                RunTrialLock(
                    trial_id=_short_id(
                        "trial",
        ...
        executions.append(
            RunExecutionLock(
                execution_id=execution_id,
                cell_digest=planned_execution.cell_digest,
                deployment_digest=planned_execution.deployment_digest,
                model=planned_execution.model,
                deployment=planned_execution.deployment,
                agent=planned_execution.agent,
                provider=planned_execution.provider,
                max_concurrent_requests=planned_execution.max_concurrent_requests,
                spend_cap_microusd=planned_execution.spend_cap_microusd,
                estimated_wave_cost_microusd=(
                    planned_execution.estimated_wave_cost_microusd
                ),
                shards=shards,
            )
        )
    initial_waves = _locked_initial_waves(plan, executions)
    return RunLock(
        run_id=run_id,
...
def _locked_initial_waves(
    ...
    grouped: dict[str, list[RunShardLock]] = {}
    for execution in sorted(executions, key=lambda value: value.execution_id):
        grouped.setdefault(execution.deployment_digest, []).extend(
            sorted(execution.shards, key=lambda value: value.shard_id)
        )
...
def build_wave_lock(
    ...
    *,
    ...
    expected_run = build_run_lock(
        build_run_plan(
            spec,
            source_lock=run.source_lock,
    ...
    if run != expected_run:
        raise ValueError("run lock does not match the resolved manifest")
    _validate_submit_wave_action(run, action)

    resolved_spec = resolved_experiment(spec, run.source_lock)
    selected = _selected_wave_shards(run, action)
    selected_trial_ids = {
        trial.trial_id
        for _execution, shards in selected
        for shard in shards
    ...
    for execution, shards in selected:
        ...
        configuration = build_execution_lock(
            bound_spec,
        ...
        execution_locks.append(
            WaveExecutionLock(
                artifact_prefix=(
                    f"{run.artifact_prefix}/executions/{execution.execution_id}"
                ),
    ...
    duration_seconds = _wave_duration_seconds(
        run, resolved_spec, action, provider_target
    )
    return WaveLock(
        ...
        artifact_prefix=(
            f"{run.artifact_prefix}/waves/{deterministic_wave_id(action.action_key)}"
        ),
        max_shards=run.max_shards_per_wave,
...
def managed_wave_endpoint(
    ...
    matches = [
        ...
        if execution.deployment_digest == deployment
    ]
    if not matches:
        raise ValueError("run does not contain the deployment digest")
    profile_pairs = {(execution.model, execution.deployment) for execution in matches}
    ...
    if deployment_digest(model, profile) != deployment:
        raise ValueError("manifest deployment does not match the run lock")
...
def _bound_wave_spec(
    ...
    profile = next(
        ...
        if profile.id == execution.deployment
    )
    if isinstance(profile, ProviderTarget):
...
def _validate_submit_wave_action(run: RunLock, action: SubmitWaveAction) -> None:
    if action.kind not in {"submit-wave", "retry-shard"} or action.run_id != run.run_id:
        raise ValueError("wave action does not target the run")
    ...
    if len(action.shard_ids) > run.max_shards_per_wave:
        raise ValueError("wave action exceeds the run shard bound")
...
def _selected_wave_shards(
    run: RunLock, action: SubmitWaveAction
) -> list[tuple[RunExecutionLock, list[RunShardLock]]]:
    requested = set(action.shard_ids)
    selected: list[tuple[RunExecutionLock, list[RunShardLock]]] = []
    found: set[str] = set()
    for execution in run.executions:
        shards = [shard for shard in execution.shards if shard.shard_id in requested]
        if not shards:
            continue
        if execution.deployment_digest != action.deployment_digest:
            raise ValueError("wave action mixes incompatible deployment digests")
        selected.append((execution, shards))
        found.update(shard.shard_id for shard in shards)
    missing = requested - found
    if missing:
        raise ValueError("wave action references an unknown run shard")
    return selected
...
def run_json_schemas() -> dict[str, dict[str, object]]:
    return {
        "run_plan": RunPlan.model_json_schema(),
        "run_lock": RunLock.model_json_schema(),
