...
from harbor_hf.run_observer import BucketRunObserver, RunObserver
...
class RunApplyError(RuntimeError):
    """Raised when a run pass cannot be applied safely."""


class ActionExecutionError(RunApplyError):
    """Raised when a remote action is known not to have succeeded."""


class AmbiguousActionOutcome(RunApplyError):
...
class RunApplyResult(FrozenModel):
    run_id: str
    plan: ReconcilePlan
    applied: list[AppliedAction]


class RunApplyFailure(FrozenModel):
    run_id: str
    error_type: str
    message: str
...
class RunPublicationPort(Protocol):
        ...
        *,
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="harbor-hf-wave-") as name:
                ...
                _write_model(staging / "run.lock.json", run)
...
def _missing_observed_events(
    durable_events: Sequence[RunEvent], observed_events: Sequence[RunEvent]
) -> list[RunEvent]:
    durable_by_id = {event.event_id: event for event in durable_events}
    missing: list[RunEvent] = []
...
class RunReconciler:
    """Apply one bounded, stateless run reconciliation pass."""
    ...
    def __init__(
        ...
        *,
        ...
        result_publisher: RunPublicationPort | None = None,
    ...
    def refresh_observation(
        self, run_id: str
    ) -> tuple[RunLock, list[RunEvent], bytes, ExperimentSpec]:
        lock, events = self.store.load_run(run_id)
        ...
        if missing and self.store.ensure_events(run_id, missing):
            lock, events = self.store.load_run(run_id)
        return lock, events, request, spec
    ...
    def apply_run(
        ...
        *,
        ...
        if (
            ...
            return RunApplyResult(run_id=run_id, plan=plan, applied=[])
        pending = _pending_actions(projection.run.actions, reservations)
        selected = sorted(
        ...
        allow_billable = projection.run.status in {"queued", "active"}
    ...
    def _apply_selected(
        ...
        *,
        ...
        for action in selected:
            ...
            path = action_claim_path(lock.run_id, action.action_id)
            try:
    ...
    def _action_claim_owner(
        self, lock: RunLock, action: ReconcileAction
    ) -> dict[str, str]:
    ...
    def apply_all(
        ...
        *,
    ...
    ) -> list[RunApplyResult | RunApplyFailure]:
        results: list[RunApplyResult | RunApplyFailure] = []
        selected_run_ids = (
            list(dict.fromkeys(run_ids))
            if run_ids is not None
            else self.store.list_runs()
        )
        observed: dict[str, AdmissionUsage] = {}
        admission_unknown = False
        for run_id in selected_run_ids:
            try:
                observed[run_id] = self._admission_usage(run_id)
            except Exception:
                admission_unknown = True
                observed[run_id] = AdmissionUsage()
        ...
        for run_id in selected_run_ids:
            try:
                run_context = baseline.model_copy(
                    update={
                ...
                result = self.apply_run(run_id, context=run_context)
    ...
    def _admission_usage(self, run_id: str) -> AdmissionUsage:
        ...
        return _build_admission_usage(run_id, projection, reservations)
    ...
    def _recover_terminal_jobs(
        ...
        changed = False
        for action_id, action_state in sorted(projection.run.actions.items()):
    ...
    def _recover_lost_executions(
        ...
        for execution in projection.attempts.values():
            ...
            self._record_durable_event(
                ...
                subject_id=execution.attempt_id,
                kind="attempt.failed",
                ...
                identity=f"{execution.attempt_id}:lost:{job.job_id}",
            )
            changed = True
        return changed
    ...
    def _reserve_new(
        ...
        for action in actions:
            ...
            if self.store.reserve_action(lock.run_id, record, event):
        ...
        *,
        ...
        if action.kind == "publish-summary":
            if terminal_decision is None:
                raise RunApplyError("summary action has no terminal decision")
    ...
    def _exhaust_trials(
        ...
        if projection.cancel_requested_at is not None:
            raise ActionExecutionError("run cancellation superseded retry exhaustion")
        ...
        for trial_id in action.trial_ids:
            ...
            event_identity = hashlib.sha256(
                f"{lock.run_id}:{trial_id}:spend-cap-exhausted".encode()
            ).hexdigest()[:32]
            events.append(
                new_event(
                    subject_type="trial",
        ...
        try:
            self.store.ensure_events_unless_cancelled(lock.run_id, events)
        except RunCancellationWon as error:
            raise ActionExecutionError(str(error)) from error
    ...
    def _publish_summary(
        ...
        if decision is None or self.finalizer is None:
            raise ActionExecutionError("run summary finalization is not configured")
        try:
            self.finalizer.finalize(lock, spec, projection, decision)
        except RunFinalizationError as error:
            raise ActionExecutionError(str(error)) from error
        ...
        try:
            self.result_publisher.publish(lock.run_id)
        except (OSError, RuntimeError, ValueError) as error:
            raise ActionExecutionError(
                f"run result publication failed: {error}"
            ) from error
        return decision.summary_path

    def _publish_results(self, lock: RunLock) -> str:
        if self.result_publisher is None:
            raise ActionExecutionError("automatic result publication is not configured")
        try:
            self.result_publisher.publish(lock.run_id)
    ...
    def _submit_wave(
        ...
        *,
        ...
        if not allow_submission:
            raise ActionExecutionError(
                "run cancellation superseded the unsubmitted wave action"
            )
        ...
        return self.jobs.submit(wave, request=request, run=lock).job_id
    ...
    def _cancel_execution(
        ...
        attempt_id = action.target_ids[0]
        execution = projection.attempts.get(attempt_id)
        if execution is None or execution.status != "active":
            raise ActionExecutionError(
                f"execution cancellation target is not active: {attempt_id}"
            )
        if execution.wave_id is None:
            raise ActionExecutionError(
                f"execution cancellation target has no wave: {attempt_id}"
            )
        ...
        self._record_durable_event(
            ...
            payload=AttemptOutcomePayload(
                trial_id=execution.trial_id,
                physical_attempt=execution.physical_attempt,
                message="cancelled by run reconciliation",
            ),
            identity=f"{attempt_id}:cancelled:reconciler",
        )
        return remote_id or attempt_id
    ...
    def _drain_wave(
        ...
        if projection.run.status == "cancel_requested":
            self._record_durable_event(
                lock,
                subject_type="run",
                subject_id=lock.run_id,
                kind="run.draining",
                payload=LifecyclePayload(message="run cancellation is draining"),
                identity=f"{lock.run_id}:draining:reconciler",
            )
        return wave_id

    def _manual_intervention(self, lock: RunLock, action: ReconcileAction) -> str:
        wave_id = action.wave_id or f"wave-{action.action_key}"
        self._record_durable_event(
            lock,
            subject_type="run",
            ...
            identity=f"{lock.run_id}:{wave_id}:manual-intervention",
        )
        return wave_id
    ...
    def _record_wave_closed(
        ...
        for kind in kinds:
            ...
            identity = hashlib.sha256(
                f"{lock.run_id}:{wave_id}:{kind}:reconciler".encode()
            ).hexdigest()[:32]
    ...
    def _record_durable_event(
        ...
        lock: RunLock,
        *,
        subject_type: Literal["run", "trial", "attempt", "wave"],
        subject_id: str,
        kind: EventKind,
        payload: LifecyclePayload | AttemptOutcomePayload | WaveLifecyclePayload,
        identity: str,
    ) -> None:
        observed_at = self._next_observed()
        event_identity = hashlib.sha256(
            f"{lock.run_id}:{identity}".encode()
        ).hexdigest()[:32]
        self._ensure_durable_event(
            lock,
            new_event(
    ...
    def _ensure_durable_event(self, lock: RunLock, event: RunEvent) -> None:
        if self.store.ensure_event(lock.run_id, event):
            return
        _lock, events = self.store.load_run(lock.run_id)
    ...
    def _record_terminal(
        ...
        event = new_event(
            ...
            identifier=lambda: hashlib.sha256(
                (f"{lock.run_id}:{decision.status}:{decision.summary_path}").encode()
            ).hexdigest()[:32],
        )
        self.store.ensure_event(lock.run_id, event)
    ...
    def _record_outcome(
        ...
        *,
        ...
        self.store.append_event(lock.run_id, event)
...
def _validate_spend_exhaustion_targets(lock: RunLock, action: ReconcileAction) -> None:
...
def _context_with_unobserved_actions(
    ...
    if not unobserved:
        return baseline
    usage = _usage_from_admissions(lock.run_id, 0, unobserved)
...
def _usage_from_admissions(
    ...
    return AdmissionUsage(
        ...
        run_active_waves={run_id: active} if active else {},
        run_spend_microusd={run_id: spend} if spend else {},
    )
...
def _active_action_admissions(
    ...
    for action_id, state in projection.run.actions.items():
        ...
        action = reservations.get(action_id)
        if action is None:
            raise RunApplyError(f"action event has no reservation record: {action_id}")
        wave_id = action.wave_id or f"wave-{action.action_key}"
        observed = projection.waves.get(wave_id)
...
def _can_recover_orphaned_endpoint(
    ...
    provenance_actions = {
        state.action_id
        for state in projection.run.actions.values()
...
def _action_targets_deployment(
    lock: RunLock, action: ActionProjection, deployment_digest: str
) -> bool:
...
def _combined_usage(
    ...
    runs = dict(baseline.run_active_waves)
    spend = dict(baseline.run_spend_microusd)
    ...
    for contribution in contributions:
        ...
        merge(runs, contribution.run_active_waves)
        merge(spend, contribution.run_spend_microusd)
    return AdmissionUsage(
        global_active_waves=global_waves,
        deployment_active_waves=deployments,
        provider_active_waves=providers,
        run_active_waves=runs,
        run_spend_microusd=spend,
    )


def hugging_face_run_reconciler(
    namespace: str,
    *,
    ...
    """Compose production HF adapters around the run application layer."""
    if store is None:
        from harbor_hf.control import HubRunStore

        store = HubRunStore(namespace)
    ...
    from harbor_hf.operations import AutomaticRunPublisher, DatasetRepositoryApi
    ...
    token = get_token()
    if token is None:
        raise RunApplyError("HF token is required for run reconciliation")
    ...
    result_publisher = AutomaticRunPublisher(
        namespace=namespace,
        store=store,
        reader=reader,
    ...
    return RunReconciler(
        ...
        observer=BucketRunObserver(reader),
        finalizer=BucketRunFinalizer(reader, writer),
        result_publisher=result_publisher,
        cleanup=lambda: None,
    )


def _validated_request(lock: RunLock, request: bytes) -> ExperimentSpec:
    try:
        spec = load_experiment_bytes(request, source="run request")
    except ManifestError as error:
        raise RunApplyError("run request is not a valid manifest") from error
    try:
        ...
        raise RunApplyError(
            "run request cannot reproduce its immutable lock"
        ) from error
    if expected != lock:
        raise RunApplyError("run request does not match its immutable lock")
    return spec
    ...
    *,
    ...
    for value in values:
        try:
            ...
            raise RunApplyError("action reservation is malformed") from error
        if action.run_id != run_id or action.action_id != f"act-{action.action_key}":
            raise RunApplyError("action reservation identity is malformed")
        if action.action_id in reservations:
            raise RunApplyError("action reservation ID is duplicated")
...
def _wave_provider(lock: RunLock, deployment_digest: str) -> str:
    provider = next(
        (
            run.provider
            for run in lock.executions
...
def _remote_namespace(spec: ExperimentSpec) -> str:
    if spec.remote is None:
        raise ActionExecutionError("run manifest has no remote execution")
