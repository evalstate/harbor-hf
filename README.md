...
- Only the root-owned bridge can read `HF_INFERENCE_TOKEN`. Task processes receive a loopback inference URL, and the bridge enforces the locked model, request size, output token, and concurrency limits. It records root-owned provider token totals so every Chat Completions harness has consistent usage and cost metrics even when its CLI omits them.
