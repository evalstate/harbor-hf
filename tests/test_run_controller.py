...
from harbor_hf.run_apply import RunReconciler
from harbor_hf.run_controller import (
    InProcessWaveExecutor,
    RunController,
    RunControllerError,
)
from harbor_hf.run_input import (
    ValidatedRunInput,
    validate_run_input,
    write_run_input,
)
...
class MemoryRunStore:
    def __init__(self, lock: RunLock, request: bytes) -> None:
        self.lock = lock
...
class CompletingReconciler:
    def __init__(self, store: MemoryRunStore) -> None:
        self.store = store
        self.calls = 0
        self.refresh_calls = 0

    def refresh_observation(self, run_id: str) -> None:
        assert run_id == self.store.lock.run_id
...
class FailingReconciler(CompletingReconciler):
    def apply_run(
        ...
        *,
        ...
        del run_id, context, expected_action_id
        self.calls += 1
    ...
    def recover_provider_capacity(
        ...
        *,
        ...
        if (
            ...
            and observed.run_id == replacement.run_id
...
def _runtime(
    ...
    input_manifest = write_run_input(input_root, request=request, lock=lock)
    validated = validate_run_input(input_root)
    store = MemoryRunStore(lock, request)
...
def _controller(
    ...
    *,
    ...
    from harbor_hf.run_input import ValidatedRunInput

    return RunController(
        store=cast(RunStore, store),
        state_store=cast(ControllerStateStore, state),
        reconciler=cast(RunReconciler, reconciler),
        wave_executor=cast(InProcessWaveExecutor, executor),
        validated_input=cast(ValidatedRunInput, validated),
...
def test_controller_pauses_when_observed_trial_duration_breaks_the_lock(
    ...
    events = (
        ...
        / "executions/run/trials/trial/attempts/execution/events.jsonl"
    )
    ...
    events.write_text(
        "\n".join(
            json.dumps(value)
            for value in (
                {"event": "attempt_started", "at": NOW.isoformat()},
                {
...
def test_controller_drains_one_action_then_honors_run_cancellation(
    tmp_path: Path,
    remote_spec: ExperimentSpec,
) -> None:
...
def test_controller_rejects_an_unreserved_attempt_before_claiming(
    ...
    with pytest.raises(RunControllerError, match="was not reserved"):
        _controller(
            tmp_path,
            store,
            state,
