...
) -> tuple[RunLock, bytes, WaveLock, EndpointRef]:
...
def test_submit_process_error_is_ambiguous(remote_spec: ExperimentSpec) -> None:
    ...
    with pytest.raises(
        ...
        adapter.submit(wave, request=request, run=lock)
...
def test_reservation_validation_rejects_malformed_records(
    ...
    with pytest.raises(RunApplyError, match="^action reservation is malformed$"):
        _reconciler(store, FakeEndpoints(), FakeJobs()).apply_run(lock.run_id)


def test_reservation_validation_rejects_foreign_run_identity(
    remote_spec: ExperimentSpec,
) -> None:
    lock, request, submitted = _run(remote_spec)
    action = plan_reconciliation(lock, [submitted], now=NOW)[1].actions[0]
    ...
    with pytest.raises(
        RunApplyError, match="^action reservation identity is malformed$"
    ):
        _reconciler(store, FakeEndpoints(), FakeJobs()).apply_run(lock.run_id)
...
def test_reservation_validation_rejects_duplicated_action_ids(
    ...
    with pytest.raises(RunApplyError, match="^action reservation ID is duplicated$"):
        _reconciler(store, FakeEndpoints(), FakeJobs()).apply_run(lock.run_id)
...
def test_manual_intervention_action_records_the_durable_run_transition(
    remote_spec: ExperimentSpec,
) -> None:
    lock, request, submitted = _run(remote_spec)
    ...
    assert any(
        ...
        and event.subject_id == lock.run_id
