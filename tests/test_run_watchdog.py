...
from harbor_hf.run_watchdog import (
    ControllerJob,
    RunWatchdogError,
    SnapshotRunStore,
    _controller_jobs,
...
def _lock(remote_spec: ExperimentSpec) -> RunLock:
    return build_run_lock(
        build_run_plan(_spec(remote_spec)), "run-one", clock=lambda: NOW
    )


def _status(
    lock: RunLock, state: str = "failed-infrastructure", attempt: int = 1
) -> ControllerStatus:
...
def _claim(lock: RunLock, *, live: bool) -> ControllerClaim:
...
class WatchdogStore:
    def __init__(self, lock: RunLock, spec: ExperimentSpec) -> None:
        self.lock = lock
        self.spec = spec
...
class WatchdogState:
    ...
    def read_status(self, run_id: str) -> ControllerStatus:
        assert run_id == self.lock.run_id
        return self.status

    def read_claim(self, run_id: str) -> None:
        assert run_id == self.lock.run_id
        return None
...
def test_watchdog_releases_abandoned_provider_capacity_without_recovery(
    ...
    lock = build_run_lock(build_run_plan(spec), "run-one", clock=lambda: NOW)
    store = WatchdogStore(lock, spec)
    state = WatchdogState(lock, spec)
    state.status = _status(lock, "paused-policy")
    provider = lock.executions[0].provider
    ...
    class RejectRunner:
        def run_text(self, command: list[str]) -> str:
            del command
            raise AssertionError("policy-blocked run must not launch")

    result = run_watchdog(
        lock.run_id,
        store=cast(SnapshotRunStore, store),
...
def test_watchdog_rejects_duplicate_physical_attempt_jobs(
    ...
    with pytest.raises(RunWatchdogError, match="multiple controller Jobs"):
